...
```bash
❯ cat $(which vllm)
#!/Users/maochenxiao/workspace/aiengineer/projects/personal/vllm/.venv/bin/python3
# -*- coding: utf-8 -*-
import sys
from vllm.entrypoints.cli.main import main
...
API Server 创建 Engine 时，调用`vllm/v1/engine/async_llm.py`的`AsyncLLM.from_vllm_config()`，进入`AsyncLLM`类的初始化方法。
...
vllm/v1/engine/utils.py
    launch_core_engines
    CoreEngineProcManager.__init__      // <- CoreEngine进程的进程对象被创建
python3.12/multiprocessing/process.py
    BaseProcess.start                   // <- 启动CoreEngine进程
```

CoreEngine进程对象的创建在`CoreEngineProcManager.__init__`，关键代码如下：
...
最后，在`BaseProcess.start`启动CoreEngine进程。
...
可以看到，伴随着model executor的创建，会启动Worker进程。model executor在CoreEngine进程中，但是Worker是另一个进程，这又一次跨越了进程边界。
